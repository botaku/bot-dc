...
module.exports = async (client, message) => {
    ...
    if (!timestamps.has(member.id)) {
        ...
        if ( now < expirationTime) {
            ...
            return message.channel.send(`Calm down dude.please wait **${timeLeft.toFixed(1)}** seconds to try the commands ${cmd} again.`)
        }
    ...
    try {
        ...
        console.log(`${sender.tag} (${sender.id}) run a command: ${cmd}`)
    }
}